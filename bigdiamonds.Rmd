---
title: "priceprediction with bigdiamonds dataset"
...
### Cleaning and Formating the bigdiamonds datasets
...
# adding a new column to the dataframe: xyz is another way of accessing the size of a diamond
...
ggplot(aes(x= price), data= diamondsbig)+geom_histogram(bins=20)+scale_x_log10()
ggplot(aes(x= price), data= diamondsbig)+geom_histogram(bins=50)+scale_x_log10()
ggplot(aes(x= price), data= diamondsbig)+geom_histogram(bins=100)+scale_x_log10()
...
ggplot(aes(x= xyz, y= price), data= bigdiamonds) + geom_point(alpha=0.05)+scale_y_log10()
...
### Building the linear model with a smaller subset
...
### Three factor variable: clarity, color and cut

ggplot(aes(x=xyz, y=price, color=clarity), data = bigdia_samp) + scale_x_continuous(lim=c(3,quantile(bigdia_samp$xyz,0.99)))+ geom_point() + scale_y_log10(lim=c(200,quantile(bigdia_samp$price,0.99)))+ ggtitle('xyz+Clarity Vs Price') + scale_color_brewer(palette = "Greens", type = 'div',  guide = guide_legend(reverse = T, title = 'clarity'))
...
ggplot(aes(x=xyz, y=price, color=color), data = bigdia_samp) + scale_x_continuous(lim=c(3,quantile(bigdia_samp$xyz,0.99)))+ geom_point() + scale_y_log10(lim=c(200,quantile(bigdia_samp$price,0.99)))+ ggtitle('XYZ+Color Vs Price') + scale_color_brewer(palette = "Blues", type = 'div',
...
ggplot(aes(x=xyz, y=price, color=cut), data = bigdia_samp) + scale_x_continuous(lim=c(3,quantile(bigdia_samp$xyz,0.99)))+ geom_point(alpha=0.2) + scale_y_log10(lim=c(200,quantile(bigdia_samp$price,0.99)))+ ggtitle('XYZ+Cut Vs Price') + scale_color_brewer(palette = "Set1", type = 'div',
...
### Building a linear model with the bigdia_samp
